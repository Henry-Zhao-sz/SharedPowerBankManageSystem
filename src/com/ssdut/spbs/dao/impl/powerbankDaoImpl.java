...
public class powerbankDaoImpl implements powerbankDao {
    ...
    public int savePB(powerbank pb1){
        ...
        loc loc1 = lsi.showSpeLoc(pb1.getPbLoc());
        if(loc1==null) {
        	return -3;
        }
        if(loc1.getAvailNum()==loc1.getMaxVolume()) {
        	return -2;
        }
        if(pbx!=null){
            return -1;
        ...
        try {
            conn = JdbcUtil.getConnection();
            st = conn.prepareStatement("insert into powerbank(pbID,blState,restPower,useTimeLong,pbLoc,HealthState) values(?,1,100,0,?,100)");
            st.setInt(1,pb1.getPbID());
            //st.setInt(2,pb1.getBlState());
            //st.setDouble(3,pb1.getRestPower());
            //st.setTime(4,pb1.getUseTimeLong());
            st.setString(2,pb1.getPbLoc());
